...
import type {
  State as PhraseTranslationsState,
  PhraseTranslationId,
  PhraseTranslation,
...
export const getSentences = (state: AppState): Array<Sentence> =>
  state.tpSentences;

export const getWord = (state: AppState, wordId: WordId): Word =>
  state.tpWords[wordId];
const getWordIndex = (state, wordId): number =>
  (getWord(state, wordId) || { index: -1 }).index;
export const getSentenceFromWord = (
  state: AppState,
  wordId: WordId
): Sentence => getSentences(state)[state.tpWords[wordId].sentence];

export const getEnSentence = (
  state: AppState,
  index: number
): SentenceTranslation => enSentences.getEnSentence(state.enSentences, index);

export const getHighlightedWord = (state: AppState): Word | null => {
...
export const wasSelectionMade = (state: AppState): boolean =>
  mouse.wasSelectionMade(state.mouse);
...
export const isWordInPendingSelection = (
  state: AppState,
  wordId: WordId
): boolean => {
...
export const getSelection = (state: AppState): Word | null => {
  const id = mouse.getSelection(state.mouse).start;
  return id ? getWord(state, id) : null;
};

export const getWordColor = (state: AppState, wordId: WordId): Color =>
  state.colors[getWord(state, wordId).sentence][
    getIndexInSentence(state, wordId)
  ];
...
export const lookUpTranslation = (
  ...
  return phraseTranslationId
    ? getPhraseTranslation(state, phraseTranslationId)
    : null;
};
export const lookUpTranslations = (
  ...
  const result: PhraseTranslation[] = [];
  for (const id in state.phraseTranslations) {
    ...
    if (
      enLemma &&
      tpLemmaId === translation.tpLemmaId &&
      (!enPartsOfSpeech || enPartsOfSpeech.some((pos) => enLemma.pos === pos))
    ) {
...
export const getTpLemmaId = (
  state: AppState,
  text: string,
  pos: TokiPonaPartOfSpeech
): TpLemmaId | null | undefined => tpLemmas.getId(state.tpLemmas, text, pos);
export const getTpLemmaText = (state: AppState, tpLemmaId: TpLemmaId): string =>
  tpLemmas.getText(state.tpLemmas, tpLemmaId);
export const isNewProperNoun = (
  state: AppState,
  tpLemmaId: TpLemmaId
): boolean => !Number.isInteger(tpLemmaId);
...
export const getEnWord = (state: AppState, id: EnWordId) =>
  enWords.getEnWord(state.enWords, id);
export const getEnWordText = (state: AppState, id: EnWordId) =>
  enWords.getEnWordText(state.enWords, id);
export const getEnWordFromTp = (state: AppState, wordId: WordId) =>
  enWords.getEnWordFromTp(state.enWords, wordId);
