...
const darken = ([h, s, l]: Color, level: number) => [
  h + level * 4,
  s - level * 15,
  l - level * 10,
];
...
const nextWord = (sentenceWords: Array<WordId>, word: WordId): WordId =>
  sentenceWords[sentenceWords.indexOf(word) + 1];
const getPhraseHead = (
  words: TpWordsState,
...
const getBaseColor: GetBaseColor = (words, sentenceWords, word, level = 0) => {
  ...
  return BASE_COLORS[role]
    ...
    : getBaseColor(
        words,
        sentenceWords,
        getPhraseHead(words, sentenceWords, word),
        level + 1
      );
};
